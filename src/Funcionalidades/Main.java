...
public class Main {
  ...
  public static int utilizarItem(Jogador aliados, Jogador inimigos) {
    borda();
    mostraItens(aliados);
    borda();

    int opInt, escolhidoInt;
    String escolhido, op;

    op = JOptionPane.showInputDialog(aliados.getNome() + " escolha um item para utilizar:");
    try {
      opInt = Integer.parseInt(op);

      if (opInt >= 0 && opInt < aliados.getInventario().size()) {
        if (aliados.getInventario().get(opInt) instanceof Espada || aliados.getInventario().get(opInt) instanceof Pocao
            || aliados.getInventario().get(opInt) instanceof Couraca
            || aliados.getInventario().get(opInt) instanceof Estilingue) {
          printaPersonagens(aliados);
          escolhido = JOptionPane
              .showInputDialog(aliados.getNome() + " escolha um personagem aliado para utilizar o item escolhido");
          escolhidoInt = Integer.parseInt(escolhido);
          if (escolhidoInt >= 0 && escolhidoInt < aliados.getPersonagens().size()) {
            aliados.getInventario().get(opInt).efeito(aliados.getPersonagens().get(escolhidoInt));
          } else {
            JOptionPane.showMessageDialog(null, "Não existem personagens nessa posição");
            return 0;
          }
        } else {
          printaPersonagens(inimigos);
          escolhido = JOptionPane
              .showInputDialog(aliados.getNome() + " escolha um personagem inimigo para utilizar o item escolhido");
          escolhidoInt = Integer.parseInt(escolhido);
          if (escolhidoInt >= 0 && escolhidoInt < inimigos.getPersonagens().size()) {
            aliados.getInventario().get(opInt).efeito(inimigos.getPersonagens().get(escolhidoInt));
          } else {
            JOptionPane.showMessageDialog(null, "Não existem personagens nessa posição");
            return 0;
          }
        ...
        JOptionPane.showMessageDialog(null, "Não existem itens nessa posição");
        return 0;
      }
    } catch (Exception ex) {
      JOptionPane.showMessageDialog(null, "Entrada inválida");
    ...
    JOptionPane.showMessageDialog(null, "O item " + aliados.getInventario().get(opInt).getNome() + " foi usado");
    if (aliados.getInventario().get(opInt).getDurabilidade() <= 0) {
      JOptionPane.showMessageDialog(null, "O item " + aliados.getInventario().get(opInt).getNome() + " quebrou");
      aliados.removerItem(opInt);
    } else {
      aliados.getInventario().get(opInt).setDurabilidade(aliados.getInventario().get(opInt).getDurabilidade() - 1);
    }
    return 1;
  }
  ...
  public static void batalha(Jogador j1, Jogador j2, Tabuleiro tabuleiro, ArrayList<Item> sorteioItem)
    ...
    while (jogadorAtual.getPersonagens().size() != 0 && jogadorProxRodada.getPersonagens().size() != 0
      ...
      if (passouTurno(ehVezDeJ1, jogadorAtual, j1, j2)) {
        if (!jogadorAtual.getInventario().isEmpty()) {
          while (true) {
            escolhido = JOptionPane.showInputDialog(jogadorAtual.getNome() + ", deseja usar algum item ? (S/N)");

            if (escolhido.equals("s") || escolhido.equals("S")) {
              if (utilizarItem(jogadorAtual, jogadorProxRodada) == 1) {
                break;
              }
            } else if ((escolhido.equals("n") || escolhido.equals("N"))) {
              break;
            } else {
              JOptionPane.showMessageDialog(null, "Entrada inválida");
            }
      ...
      if (escolhido.equals("1")) {
        ...
        try {
          ...
          if (jogadorAtual.getPersonagens().get(opInt).getCargaEspecial() > 0) {
            ...
            } else if (jogadorAtual.getPersonagens().get(opInt) instanceof CientistaMaluco) {
              printaPersonagens(jogadorProxRodada);
              atacado = JOptionPane.showInputDialog(jogadorAtual.getNome() + " escolha o personagem para atacar");
              atacadoInt = Integer.parseInt(atacado);
